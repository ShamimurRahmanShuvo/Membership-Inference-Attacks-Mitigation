...
def train_target_model(dataset, epochs=100, batch_size=100, learning_rate=0.01, l2_ratio=1e-7,
    ...
    attack_x1 = np.vstack(attack_x)
    attack_y = np.concatenate(attack_y)
    attack_x1 = attack_x1.astype('float32')
...
def train_attack_model(dataset=None, n_hidden=50, learning_rate=0.01, batch_size=200, epochs=50,
    ...
    c_pred_y = train_model1(dataset, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
    ...
    print(true_y)
    print(pred_y)
    print ('Testing Accuracy: {}'.format(accuracy_score(test_y, c_pred_y)))
    print (classification_report(test_y, c_pred_y))

def save_data():
    print ('-' * 10 + 'SAVING DATA TO DISK' + '-' * 10 + '\n')
    
    x, y, test_x, test_y = load_dataset(args.train_feat1, args.train_label, args.test_feat, args.train_label)
    if test_x is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('train_feat1', type=str,default='train_feat')
