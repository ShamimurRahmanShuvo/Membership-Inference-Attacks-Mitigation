...
from classifier import train1 as train_model1
from classifier import train2 as train_model2
...
def get_data_indices(data_size, target_train_size=int(2e4), sample_target_data=True):
...
def train_target_model(dataset, epochs=1, batch_size=1, learning_rate=0.01, l2_ratio=1e-7,
    ...
    output_layer = train_model(dataset, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
                               batch_size=batch_size, model=model, l2_ratio=l2_ratio, noise =False ) #if no noise change train_model1 to train_model
    # test data for attack model
    attack_x, attack_y = [], []
    input_var = T.matrix('x')
    prob = lasagne.layers.get_output(output_layer, input_var, deterministic=False)
    ...
    attack_x.sort()
    #print('Shape after:', attack_x.shape)
    
    #Confidence Score
    #attack_x = np.delete(attack_x,attack_x[:]>0.7,axis = 0)
    #print("Length of Attack_x", len(attack_x))
    # for i in range(len(attack_x)):
    #     if np.amax(attack_x[i,:])<0.80:
    #         attack_x[i,:]=0
    #     else:
    #         attack_x[i,:]=attack_x[i,:]
    
    print("XAttack ", attack_x)
    print("XAttack Shape ", attack_x.shape)
...
def train_shadow_models(n_hidden=50, epochs=100, n_shadow=20, learning_rate=0.01, batch_size=100, l2_ratio=1e-7,
                        model='nn', save=True):
    ...
    for i in range(n_shadow):
        ...
        output_layer = train_model(data, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
                                   batch_size=batch_size, model=model, l2_ratio=l2_ratio, noise=False)
...
def train_attack_model(classes, dataset=None, n_hidden=50, learning_rate=0.01, batch_size=200, epochs=50,
    ...
    for c in unique_classes:
        ...
        #print(np.amax(c_train_y))
        c_pred_y = train_model2(c_dataset, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
                               batch_size=batch_size, model=model, rtn_layer=False, l2_ratio=l2_ratio, noise=False)
    ...
    print ('Confusion Matrix of attack model:', confusion_matrix(test_y, pred_y))
...
def save_data():
    ...
    if test_x is None:
        ...
        #test_size=args.test_ratio, stratify=y
        np.save(DATA_PATH + 'chk', x)  #not mentioned in original code
...
def attack_experiment():
    print("Running for low confidence")
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--shadow_model', type=str, default='softmax')
    parser.add_argument('--target_learning_rate', type=float, default=0.00001)
    parser.add_argument('--target_batch_size', type=int, default=1)
    parser.add_argument('--target_n_hidden', type=int, default=1000)
    ...
    #parser.add_argument('--shadow_data_size', type=int, default=int(2e4))   # number of data point used in shadow model
    parser.add_argument('--shadow_model', type=str, default='nn')
    parser.add_argument('--shadow_learning_rate', type=float, default=0.00001)
    parser.add_argument('--shadow_batch_size', type=int, default=1000)
    parser.add_argument('--shadow_n_hidden', type=int, default=1000)
    parser.add_argument('--shadow_epochs', type=int, default=1000)
    parser.add_argument('--shadow_l2_ratio', type=float, default=1e-15)

    # attack model configuration
    
    parser.add_argument('--attack_model', type=str, default='nn')
    parser.add_argument('--attack_learning_rate', type=float, default=0.00001)
    parser.add_argument('--attack_batch_size', type=int, default=1000)
    parser.add_argument('--attack_n_hidden', type=int, default=1000)
    parser.add_argument('--attack_epochs', type=int, default=1000)
