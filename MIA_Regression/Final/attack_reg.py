...
#    test_y = test_y.astype(np.float64)
    # initialize
    init = Regression(train_x, train_y, test_x, test_y, iters = 10000, learning_rate = 0.5, optimizer = 'sgd', batch_size = None, 
    ...
    pred_tr = init.PREDICT(train_x)
    pred_te = init.PREDICT(test_x)

# mse train - test
    print ('mean_squared_error on train data is', str(metrics.mean_squared_error(train_y, pred_tr)))
# mean_squared_error on train data is 27.0498

    print ('mean_squared_error on test data is', str(metrics.mean_squared_error(test_y, pred_te)))
# mean_squared_error on test data is 12.7468
    #output_layer = train_model(dataset, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
                               #batch_size=batch_size, model=model, l2_ratio=l2_ratio) #if no noise change train_model1 to train_model
...
#    parser.add_argument('train_label', type=str,default='train_label')
    ...
    parser.add_argument('--target_epochs', type=int, default=1000)
    ...
    parser.add_argument('--shadow_learning_rate', type=float, default=0.0001)
